...
class MemcachedLockSelfSyncTest extends \PHPUnit_Framework_TestCase {
    ...
    public function test_MemcachedLock() {
        ...
        try {
            $MemcachedLock->acquire(MemcachedLock::LOCK_MIN_TIME_SYNC * 0.9);
            $this->assertFalse("Expect Exception " . \InvalidArgumentException::class);
        } catch (\Exception $Exception) {
            $this->assertInstanceOf(\InvalidArgumentException::class, $Exception);
        }

        $this->assertTrue($MemcachedLock->acquire(MemcachedLock::LOCK_MIN_TIME_SYNC * 2));

        try {
            $MemcachedLock->acquire(MemcachedLock::LOCK_MIN_TIME_SYNC * 2);
        ...
        $this->assertTrue($MemcachedLock->update(MemcachedLock::LOCK_MIN_TIME_SYNC * 4));
        $this->assertTrue($MemcachedLock->update(MemcachedLock::LOCK_MIN_TIME_SYNC * 3));
        $this->assertTrue($MemcachedLock->update(MemcachedLock::LOCK_MIN_TIME_SYNC * 2));
        ...
        try {
            $MemcachedLock->update(MemcachedLock::LOCK_MIN_TIME_SYNC * 2);
            $this->assertFalse("Expect " . LockException::class);
        } catch (\Exception $Exception) {
            $this->assertInstanceOf(LockException::class, $Exception);
        }
    ...
    public function test_MemcachedLock_WithoutExceptions() {
        $key = static::TEST_KEY;
        $MemcachedLock = new MemcachedLock(static::$Memcached, $key, MemcachedLock::FLAG_CATCH_EXCEPTIONS | MemcachedLock::FLAG_USE_SELF_EXPIRE_SYNC);

        $this->assertFalse($MemcachedLock->acquire(MemcachedLock::LOCK_MIN_TIME_SYNC * 0.9));
        $this->assertTrue($MemcachedLock->acquire(MemcachedLock::LOCK_MIN_TIME_SYNC * 2));
        $this->assertFalse($MemcachedLock->acquire(MemcachedLock::LOCK_MIN_TIME_SYNC * 2));
        $this->assertFalse($MemcachedLock->acquire(MemcachedLock::LOCK_MIN_TIME_SYNC * 0.9));
        ...
        $this->assertFalse($MemcachedLock->update(MemcachedLock::LOCK_MIN_TIME_SYNC * 2));
    ...
    public function test_MemcachedLock_LockTime() {
        ...
        $MemcachedLock = new MemcachedLock(static::$Memcached, $key, MemcachedLock::FLAG_USE_SELF_EXPIRE_SYNC);
        $MemcachedLock2 = new MemcachedLock(static::$Memcached, $key, MemcachedLock::FLAG_USE_SELF_EXPIRE_SYNC);

        for ($i = 1; $i <= 5; $i++) {
            $microtime = microtime(true);

            $this->assertTrue($MemcachedLock->acquire(MemcachedLock::LOCK_MIN_TIME_SYNC * $i));
            ...
            //$microtime = microtime(true);
            $this->assertTrue($MemcachedLock2->acquire(MemcachedLock::LOCK_MIN_TIME_SYNC * $i, $i + 1));
            $waitTime = microtime(true) - $microtime;

            $this->assertTrue($MemcachedLock2->update(1));

            $this->assertGreaterThan(MemcachedLock::LOCK_MIN_TIME_SYNC * ($i - 1), $waitTime);
            $this->assertLessThanOrEqual(MemcachedLock::LOCK_MIN_TIME_SYNC * ($i + 1), $waitTime);
    ...
    public function test_MemcachedLock_WaitTime() {
        ...
        for ($i = 1; $i <= 5; $i++) {
            ...
            $this->assertFalse($MemcachedLock2->acquire(
                MemcachedLock::LOCK_MIN_TIME_SYNC * $i,
                MemcachedLock::LOCK_MIN_TIME_SYNC * ($i - 1))
            );
            $this->assertTrue($MemcachedLock->release());
        }
